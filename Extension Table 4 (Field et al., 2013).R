...
run_method = function(numtrees, index, lambdas, boolean.plot, boolean.lambdas) {
  ...
  results_ATE <- basic.results
    colnames(results_ATE) <- c("ATE GRF", "ATE CR.GRF", "ATE LLCF")    
    rownames(results_ATE) <- c("business closure", "average difference in profits between high- and low-profit months",
...
results_table4 <- list('Sheet1' = results[["results_ATE"]])
write.xlsx(results_table4, file = "Table 4 (Field et al., 2013).xlsx")

...
county.clusters <- as.numeric(voting.study.data$county)
...
training.sample.size <- c(4000, 2000, 1200, 1000, 800, 600, 400)
n.test <- 2000
...
run_method = function(training.sample.size, num.reps) {
  results = sapply(training.sample.size, function(n.train) {  
    ...
    basic.results = replicate(num.reps.training, {
      ...
      # For LLCF we create the X matrix including district-specific dummies
      ...
      # Implement GRF
      GRF <- causal_forest(current.X, Ynew.train, W.train, num.trees = numtrees, honesty = TRUE, tune.parameters = "all")
      ...
      forest.W <- ll_regression_forest(as.matrix(X.train), as.vector(W.train), enable.ll.split = TRUE, ll.split.weight.penalty = TRUE,
                                       honesty = TRUE, honesty.fraction = 0.8, honesty.prune.leaves = FALSE, num.trees = 4000)
      ...
      # Implement LLCF
      LLCF <- causal_forest(current.X, Ynew.train, W.train, Y.hat = Y.hat, W.hat = W.hat, honesty = TRUE, num.trees = numtrees, honesty.fraction = 0.8, 
      ...
      CR.LLCF <- causal_forest(X.train, Ynew.train, W.train, Y.hat = Y.hat, W.hat = W.hat, honesty = TRUE, num.trees = numtrees, honesty.fraction = 0.8, 
                            clusters = county.clusters.train, honesty.prune.leaves = FALSE, tune.parameters = c("min.node.size", "sample.fraction", "mtry", "alpha", "imbalance.penalty"))
      ...
      sub.basic.results = replicate(num.reps.test, {
        ...
        # Predictions LLCF
        LLCF.pred <- predict(LLCF, current.X.test, linear.correction.variables = selected, ll.weight.penalty = TRUE, ll.lambda = 1.5, estimate.variance = TRUE)
  ...
  colnames(results) = c("In-sample RMSE GRF", "In-sample RMSE CR.GRF", "In-sample RMSE LLCF", "In-sample RMSE CR.LLCF", 
                        ...
                        "Out-of-sample Coverage GRF", "Out-of-sample Coverage CR.GRF", "Out-of-sample Coverage LLCF", "Out-of-sample Coverage CR.LLCF", 

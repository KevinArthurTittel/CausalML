...
s <- 1
num.reps <- 100
n.training <- c(200, 400, 600, 800, 1000, 1200, 2000, 4000)
d <- 25
...
run_simulation <- function(n.training, num.reps) {
  results <- sapply(n.training, function(n) {
    basic.results <- replicate(num.reps, {
      ...
      # Implement LLCF
      LLCF <- causal_forest(as.matrix(dat$X), as.numeric(dat$Y), as.numeric(dat$W), Y.hat = Y.hat, W.hat = W.hat, honesty = TRUE, honesty.fraction = 0.8, honesty.prune.leaves = FALSE, 

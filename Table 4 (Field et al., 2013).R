...
# Initialize results matrix
  resultsTable4OriginalPaper <- matrix(data = 0, nrow = 7, ncol = 4)
  resultsTable4OriginalPaper[2:13,1] <- c("business closure", "average difference in profits between high- and low-profit months",
                 "sold goods or services at a discount to make loan payment", "customers buy on credit",
  ...
  for (i in 1:6) {
    ...
    # Causal Forest estimation:
      ...
      # Compute CATE with corresponding 95% confidence intervals
        ...
        resultsTable4OriginalPaper[(i+1),2] <- paste(round(CATE[1], 3), "(", round(CATE[2], 3), ")")
    ...
    # Cluster-Robust Causal Forest estimation: 
      ...
      # Compute CATE with corresponding 95% confidence intervals
        ...
        resultsTable4OriginalPaper[(i+1),3] <- paste(round(CATE.CR[1], 3), "(", round(CATE.CR[2], 3), ")")
    ...
    resultsTable4OriginalPaper[(i+1),4] <- paste(LLCF.CATE, "(", LLCF.CATE.SE, ")")
  }

...
Grace_Period_Data <- read.dta("Downloads/112672-V1/Grace-Period-Data.dta")
...
X$sec_loan_officer <- as.numeric(factor(Grace_Period_Data$sec_loan_officer))
...
honesty.fractions <- c(0.5,0.6,0.7,0.8)
numtrees <- 4000 # Set to 1000 or 5000 to perform sensitivity analysis.
index <- c(1:5)  # Concerns 5 characteristics; do not adjust.
dep.var <- 56 # Concerns dependent variable; set to 50 for monhtly profit, 51 for log of monthly HH income, or 52 for capital.
...
for (i in index) {
  ...
  # forest.Y <- regression_forest(current.X, current.Y, num.trees = 2000, honesty = TRUE, tune.parameters = "all")
  
  # Select variables to include using preliminary GRF
  prelim.GRF <- causal_forest(current.X, current.Y, current.W, num.trees = 2000, honesty = TRUE)
  prelim.GRF.varimp <- variable_importance(prelim.GRF)
  selected.vars <- which(prelim.GRF.varimp / mean(prelim.GRF.varimp) > 0.2)
  
  # Implement GRF
  # GRF <- causal_forest(current.X[,selected.vars], current.Y, current.W, num.trees = numtrees, honesty = TRUE, tune.parameters = "all")
  GRF <- causal_forest(current.X[,selected.vars], current.Y, current.W, honesty = TRUE, honesty.fraction = 0.7, honesty.prune.leaves = FALSE, 
                       num.trees = numtrees, tune.parameters = c("min.node.size", "sample.fraction", "mtry", "alpha", "imbalance.penalty"))
  
  
  # Compute the variable importance
  GRF.varimp <- variable_importance(GRF) 
  GRF.mostimportant <- colnames(current.X)[order(GRF.varimp, decreasing = TRUE)] 
  ...
  prelim.CR.GRF <- causal_forest(current.X, current.Y, current.W, honesty = TRUE, clusters = current.loangroups, num.trees = 2000)
  ...
  CR.GRF <- causal_forest(current.X[,selected.vars], current.Y, current.W, honesty = TRUE, honesty.fraction = 0.7, honesty.prune.leaves = FALSE, 
                          clusters = current.loangroups, num.trees = numtrees)
  # Compute the variable importance
  CR.GRF.varimp <- variable_importance(CR.GRF) 
  CR.GRF.mostimportant <- colnames(current.X)[order(CR.GRF.varimp, decreasing = TRUE)]
  ...
  forest.W <- ll_regression_forest(current.X, current.W, honesty = TRUE, enable.ll.split = TRUE, ll.split.weight.penalty = TRUE, num.trees = 4000)
  W.hat <- predict(forest.W)$predictions
  forest.Y <- ll_regression_forest(current.X, current.Y, honesty = TRUE, enable.ll.split = TRUE, ll.split.weight.penalty = TRUE, num.trees = 4000)
  ...
  # Implement LLCF
  LLCF <- causal_forest(current.X, current.Y, current.W, Y.hat = Y.hat, W.hat = W.hat, honesty = TRUE, honesty.fraction = 0.7, honesty.prune.leaves = FALSE, 
                        num.trees = numtrees, tune.parameters = c("min.node.size", "sample.fraction", "mtry", "alpha", "imbalance.penalty"))
  
  # Compute the variable importance
  LLCF.varimp <- variable_importance(LLCF) 
  LLCF.mostimportant <- colnames(current.X)[order(LLCF.varimp, decreasing = TRUE)]
  ...
  # Implement LLCF
  CR.LLCF <- causal_forest(current.X, current.Y, current.W, Y.hat = Y.hat, W.hat = W.hat, honesty = TRUE, honesty.fraction = 0.7, honesty.prune.leaves = FALSE, 
                           clusters = current.loangroups, num.trees = numtrees)
  
  # Compute the variable importance
  CR.LLCF.varimp <- variable_importance(CR.LLCF) 
  CR.LLCF.mostimportant <- colnames(current.X)[order(CR.LLCF.varimp, decreasing = TRUE)]
  
  # Compute HTE with corresponding 95% confidence intervals 
  if (boolean.lambdas == FALSE) {
    # Predict: tuning without grid search over lambdas
    CR.LLCF.pred <- predict(CR.LLCF, linear.correction.variables = selected,  ll.lambda = 0.1, ll.weight.penalty = TRUE, estimate.variance = TRUE)
  ...
  results_DiffATE.new <- rbind(results_DiffATE.new, 
...
colnames(results_DiffATE.new) <- c("GRF CATE subgroup", "GRF CATE rest", "GRF p-value difference", 
                                   "CR.GRF CATE subgroup", "CR.GRF CATE rest", "CR.GRF p-value difference",
                                   "LLCF CATE subgroup", "LLCF CATE rest", "LLCF p-value difference", 
                                   "CR.LLCF CATE subgroup", "CR.LLCF  CATE rest", "CR.LLCF p-value difference","Number of observations")
rownames(results_DiffATE.new) <- c("savings", "risk loving", "wage earner", "household member chronically ill", "impatient")
results_DiffATE.new[c(2,1,4,5,3),]

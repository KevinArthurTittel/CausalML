...
treatment.group <- which(voting.study.data$treat_real == 1) 
subsample.control.group <- sample(which(voting.study.data$treat_real == 0), size = 89958, replace = FALSE)
subsample <- c(treatment.group, subsample.control.group)
...
run_method = function(training.sample.size, num.reps) {
  results = sapply(training.sample.size, function(n.train) {  
    ...
    indices.training = sample(subsample, size = n.train, replace = FALSE)
    ...
    basic.results = replicate(num.reps, {
      ...
      # Create test sample 
      indices.test <- sample(subsample[-indices.training], size = n.test, replace = FALSE)
